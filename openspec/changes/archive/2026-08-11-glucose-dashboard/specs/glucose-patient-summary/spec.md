...
El sistema SHALL calcular la adherencia de registro del período como el porcentaje de lecturas efectivamente registradas sobre las lecturas esperadas, donde las lecturas esperadas son `expected_logs_per_day` (config, default 2) multiplicado por la cantidad de días del período.
