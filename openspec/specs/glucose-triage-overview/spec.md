...
El sistema SHALL contar como "con evento Bajo reciente" a todo `UserPatient` que tenga al menos un `UserGlucoseLog` con status "Bajo - fuera de rango normal" dentro de la ventana `recent_event_window_hours` (config, default 48 horas) contadas desde el momento de la solicitud.
...
El sistema SHALL contar como "sin registro reciente" a todo `UserPatient` cuyo `UserGlucoseLog` más reciente tenga una antigüedad mayor a `recent_event_window_hours` (config, default 48 horas), o que no tenga ningún registro.
